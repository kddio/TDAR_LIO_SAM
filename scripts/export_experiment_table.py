...
def main():
    ...
    with open(args.output_md, "w", encoding="utf-8") as f:
        f.write("| Case | ATE RMSE (m) | RPE RMSE (m) | Avg TDAR Cost (ms) | Acc Residual | Gyro Residual | Fallback Rate |\n")
        f.write("| --- | --- | --- | --- | --- | --- | --- |\n")
        for r in rows:
            f.write(
                f"| {r['case']} | {r['ate_rmse_m']} | {r['rpe_rmse_m']} | {r['avg_compute_ms']} | {r['acc_residual']} | {r['gyro_residual']} | {r['fallback_rate']} |\n"
            )
