...
## TDAR-IDW Integration (IMU Endpoints + Deskew Endpoints)

This fork adds a reusable TDAR-IDW interpolator for IMU endpoint alignment in:
- `src/imuPreintegration.cpp` (optimization endpoint and reprop start endpoint)
- `src/imageProjection.cpp` (scan `timeScanCur`/`timeScanEnd` deskew endpoints)

### New Module

- `include/tdar_idw_interpolator.h`
- `src/tdar_idw_interpolator.cpp`

Interpolation model:
- Neighboring samples: select `K` nearest samples inside `window_sec`.
- Weight terms:
  - `wt = exp(-alpha * |dt|^p)`
  - `wa = exp(-beta * ||da/dt||^q)`
  - `wg = exp(-gamma * ||dw/dt||^r)` (optional)
- Combined weight:
  - `w = wt^lambda_t * wa^lambda_a * wg^lambda_g`
- State-adaptive lambda:
  - `J = ||da/dt|| + c * ||dw/dt||`
  - if `J > jerk_th`, use `lambda_high`; else `lambda_low`.
- Robust fallback to linear interpolation:
  - insufficient samples in window
  - local diff unavailable
  - `sum_w` too small
  - TDAR disabled

### Parameters

All TDAR params are in ROS param namespace `tdar_lio_sam`:
- `tdar_enable`
- `tdar_K`
- `tdar_window_sec`
- `tdar_alpha`, `tdar_beta`, `tdar_gamma`
- `tdar_p`, `tdar_q`, `tdar_r`
- `tdar_jerk_th`
- `tdar_lambda_low`, `tdar_lambda_high`
- `tdar_use_gyro_factor`
- plus robustness/diagnostic helpers:
  - `tdar_jerk_gyro_coeff`
  - `tdar_min_sum_w`
  - `tdar_diag_log`

Example file:
- `config/tdar_params_example.yaml`
...
- `[0] target_time`
- `[1] source_tag` (`imu`: 0 opt-endpoint, 1 reprop-start; `deskew`: 0 scan-start, 1 scan-end)
- `[2] state` (0 low, 1 high)
- `[3] used_k`
- `[4..6] lambda_t, lambda_a, lambda_g`
- `[7] weight_entropy`
- `[8] sum_w`
- `[9] fallback_flag`
- `[10] compute_us`
...
Script:
- `scripts/run_tdar_experiments.sh`

Usage:
```bash
scripts/run_tdar_experiments.sh <input.bag> <gt_topic> <catkin_ws> [result_dir]
```
...
## Service
  - /tdar_lio_sam/save_map
    - save map as a PCD file.
      ...
      - Example:
      ``` bash
        $ rosservice call /tdar_lio_sam/save_map 0.2 "/Downloads/LOAM/"
      ```
